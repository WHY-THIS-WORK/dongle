...
const Clubjoin = () => {
  ...
  const sendBtnHandle = () => {
    ...
    axios
      .post("http://localhost:5174/newclub", clubJoinInput)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    (async (): Promise<any> => {
      await axios.get("http://localhost:5174/category").then((res) => {
    ...
    <div className='clubjoin'>
      ...
      <div className='clubjoin_container'>
        ...
        <div className='clubjoin_input name'>
          ...
          <JoinInputValue placeholder='한/영문 12자 이하' inputHandle={inputHandle} title='name' />
        </div>
        ...
        <div className='clubjoin_input direction'>
          ...
          <JoinInputTextarea inputHandle={inputHandle} title='direction' />
        </div>
        <div className='clubjoin_input notice'>
          ...
          <JoinInputValue placeholder='게시판 이름을 입력해주세요.' inputHandle={inputHandle} title='boardName' />
        </div>
